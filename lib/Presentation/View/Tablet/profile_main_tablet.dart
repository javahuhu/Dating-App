// lib/presentation/pages/profile_page_tablet.dart
import 'dart:io';
import 'dart:convert';
...
final profileCacheProvider = StateProvider<Map<String, dynamic>?>( (ref) => null, );
...
class ProfilePageTablet extends HookConsumerWidget {
  const ProfilePageTablet({super.key});
  ...
  void _applyUserDataToUI(WidgetRef ref, dynamic user) {
    ...
    if (bioCtl.text.isEmpty ||
      ...
      bioCtl.text =
          user.bio ?? 'Looking for someone special to share life\'s adventures with. Love good conversations, weekend getaways, and trying new things!';
    }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // Keep the content width flexible; allow it to go smaller than 700 for narrow column mode.
    final double contentWidth = screenWidth.clamp(360.0, 1100.0);

    final double baseline = 1100.0;
    ...
    final bool isNarrow = screenWidth < 700.0; // <- new: single-column mode (phone-ish / narrow tablet)
    final bool isWideTablet = screenWidth >= 1000.0; // 3-column layout
    final bool isMediumTablet = !isWideTablet && !isNarrow; // 2-column stacked
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  children: [
                    SizedBox(height: s(8)),
                    _buildSidebarIcon(Icons.person, true, 'Profile', scale),
                    _buildSidebarIcon(Icons.favorite, false, 'Matches', scale),
                    _buildSidebarIcon(Icons.chat, false, 'Messages', scale),
                    _buildSidebarIcon(Icons.explore, false, 'Discover', scale),
                    _buildSidebarIcon(Icons.settings, false, 'Settings', scale),
                    const Spacer(),
                    _buildSidebarIcon(Icons.logout, false, 'Logout', scale),
                    SizedBox(height: s(8)),
                  ],
                ),
              ),
  ...
  Widget _buildThreeColumnLayout(
    ...
    TextEditingController archetypeCtrl,
    List<String> tags,
    List<PersonalityItem> personalityList,
    StateController<List<PersonalityItem>> personalityNotifier,
    List<MotiveItem> motivations,
    StateController<List<MotiveItem>> motivationsNotifier,
    List<String> frustrations,
    StateController<List<String>> frustrationsNotifier,
    TextEditingController newTraitLeftCtrl,
    TextEditingController newTraitRightCtrl,
    TextEditingController newFrustrationCtrl,
    ...
    return Row(
      ...
      children: [
        ...
        Expanded(
          flex: 2,
          child: _buildBioPersonalityCard(
            s,
            scale,
            ref,
            isEditing,
        ...
        // Right section - Motivations & Frustrations
        SizedBox(
          width: s(280),
          child: _buildMotivationsFrustrationsCard(
            s,
            scale,
            ref,
            isEditing,
  ...
  Widget _buildStackedLayout(
    ...
    return Row(
      ...
      children: [
        ...
        // Right section - Bio, Personality, Motivations & Frustrations stacked
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.stretch,
            children: [
              // Bio & Personality section
              _buildBioPersonalityCard(
                s,
                scale,
                ref,
                isEditing,
              ...
              // Motivations & Frustrations section
              Column(
                crossAxisAlignment: CrossAxisAlignment.stretch,
                children: [
                  _buildMotivationsCard(
                    s,
                    scale,
                    ref,
                    isEditing,
  ...
  // ---------- New: Column layout for narrow screens ----------
  Widget _buildColumnLayout(
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        // Profile card: make it flexible in height for narrow screens
        _buildProfileCard(
          s,
          scale,
          ref,
  ...
  Widget _buildProfileCard(
    ...
    return Container(
      // make height flexible so it doesn't overflow on narrow screens
      // a reasonable minHeight keeps the visual balance
      constraints: BoxConstraints(minHeight: s(280)),
      decoration: _cardDecoration(color: leftCardBg, radius: 16),
        ...
        children: [
          Stack(
            ...
            children: [
              ...
              GestureDetector(
                onTap: isEditing ? () => _pickAndSetAvatar(ref) : null,
                child: CircleAvatar(
                  radius: s(44),
          ...
          Container(
            ...
            decoration: _cardDecoration(color: infoBoxBg, radius: 12),
            padding: EdgeInsets.all(s(12)),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _infoRow('Age', ageCtrl, isEditing, scale),
                SizedBox(height: s(6)),
                _infoRow(
                  'Status',
                ...
                SizedBox(height: s(6)),
                _infoRow('Location', locCtrl, isEditing, scale),
                SizedBox(height: s(6)),
                Row(
                  children: [
                    SizedBox(
                      width: 80 * scale,
                      child: Text('Archetype:', style: TextStyle(fontSize: 12 * scale)),
                    ),
                    Expanded(
                      child: isEditing
                          ? TextField(
                              controller: archetypeCtrl,
                              decoration: _inputDecoration('Archetype', isDense: true, scale: scale)
                                  .copyWith(contentPadding: EdgeInsets.symmetric(horizontal: 10 * scale, vertical: 12 * scale)),
                            )
                          : Text(
                              archetypeCtrl.text,
                              style: TextStyle(
                ...
                Wrap(
                  ...
                  children: tags.isEmpty
                      ? ['Travel', 'Coffee', 'Movies', 'Fitness', 'Music', 'Cooking']
                          .map((e) => _styledInterestChip(e, kPrimaryColor, s))
                          .toList()
                      : tags
                          .map((e) => _styledInterestChip(
                                e,
                                kPrimaryColor,
                                s,
                                onDeleted: isEditing
                                    ? () {
                                        final updated = List<String>.from(tags)..remove(e);
                ...
                if (isEditing) SizedBox(height: s(10)),
                if (isEditing) _addTagField(ref, kPrimaryColor, scale),
              ],
            ),
          ),
          ...
          Align(
            ...
            child: ElevatedButton.icon(
              icon: Icon(isEditing ? Icons.save : Icons.edit, size: s(12)),
              label: Text(
  ...
  Widget _buildBioPersonalityCard(
    double Function(double) s,
    double scale,
    WidgetRef ref,
    bool isEditing,
              ...
              Expanded(
                child: isEditing
                    ? TextField(
                        controller: bioCtrl,
                        maxLines: 4,
                        decoration: _inputDecoration('Bio', scale: scale).copyWith(contentPadding: EdgeInsets.all(s(10))),
                      )
                    : Text(
                        bioCtrl.text,
                        style: TextStyle(
          ...
          child: Column(
            ...
            children: [
              ...
              ...personalityList.map((item) {
                final index = personalityList.indexOf(item);
                return Container(
                        ...
                        children: [
                          if (isEditing)
                            SizedBox(
                              width: s(100),
                              child: TextField(
                                controller: TextEditingController(text: item.left),
                                style: TextStyle(fontSize: s(11)),
                                decoration: _inputDecoration('Left', isDense: true, scale: scale),
                                onChanged: (val) => item.left = val,
                              ),
                            )
                          else
                            Text(
                              item.left,
                              style: TextStyle(fontSize: s(11), fontWeight: FontWeight.w500),
                            ),
                          if (isEditing)
                            SizedBox(
                              width: s(100),
                              child: TextField(
                                controller: TextEditingController(text: item.right),
                                style: TextStyle(fontSize: s(11)),
                                decoration: _inputDecoration('Right', isDense: true, scale: scale),
                                onChanged: (val) => item.right = val,
                              ),
                            )
                          else
                            Text(
                          ...
                          Expanded(
                            child: isEditing
                                ? Slider(
                                    ...
                                    onChanged: (val) {
                                      final updated = List<PersonalityItem>.from(personalityList);
                                      updated[index] = PersonalityItem(left: item.left, right: item.right, value: val);
                                      personalityNotifier.state = updated;
                                    },
                                  )
                                : _buildPersonalityBar(item.value),
                          ),
                          if (isEditing)
                            IconButton(
                              icon: Icon(Icons.delete, size: s(16)),
                              color: Colors.redAccent,
                              onPressed: () {
                                final updated = List<PersonalityItem>.from(personalityList)..removeAt(index);
                                personalityNotifier.state = updated;
                              },
                            ),
                        ],
                      ),
                      if (index < personalityList.length - 1) Divider(height: s(16)),
                    ],
                  ),
                );
              }),
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          Expanded(
                            child: TextField(
                              controller: newTraitLeftCtrl,
                              style: TextStyle(fontSize: s(11)),
                              decoration: _inputDecoration('Left trait', isDense: true, scale: scale),
                            ),
                          ),
                          SizedBox(width: s(6)),
                          Expanded(
                            child: TextField(
                              controller: newTraitRightCtrl,
                              style: TextStyle(fontSize: s(11)),
                              decoration: _inputDecoration('Right trait', isDense: true, scale: scale),
                            ),
                          ),
                          SizedBox(width: s(6)),
                          ElevatedButton(
                            onPressed: () {
                              if (newTraitLeftCtrl.text.trim().isNotEmpty && newTraitRightCtrl.text.trim().isNotEmpty) {
                                final updated = List<PersonalityItem>.from(personalityList)
                                  ..add(PersonalityItem(left: newTraitLeftCtrl.text, right: newTraitRightCtrl.text, value: 0.5));
                                personalityNotifier.state = updated;
                                newTraitLeftCtrl.clear();
                                newTraitRightCtrl.clear();
                              }
                            },
                            style: ElevatedButton.styleFrom(
                              backgroundColor: kPrimaryColor,
                            ...
                            child: Text(
                              'Add',
                              style: TextStyle(fontSize: s(12), color: Colors.white),
                            ),
                          ),
                        ],
                      ),
  ...
  Widget _buildMotivationsFrustrationsCard(
    double Function(double) s,
    double scale,
    WidgetRef ref,
    bool isEditing,
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        _buildMotivationsCard(s, scale, ref, isEditing, motivations, motivationsNotifier),
        SizedBox(height: s(12)),
        _buildFrustrationsCard(s, scale, ref, isEditing, frustrations, frustrationsNotifier, newFrustrationCtrl),
      ],
    );
  }

  Widget _buildMotivationsCard(
    double Function(double) s,
    double scale,
    WidgetRef ref,
    bool isEditing,
                ...
                children: [
                    ...
                    children: [
                      isEditing
                          ? SizedBox(
                              width: s(120),
                              child: TextField(
                                controller: TextEditingController(text: m.label),
                                decoration: _inputDecoration('Label', isDense: true, scale: scale),
                                onChanged: (val) {
                                  final updated = List<MotiveItem>.from(motivations);
                                  updated[idx] = MotiveItem(label: val, value: updated[idx].value);
                                  motivationsNotifier.state = updated;
                                },
                              ),
                            )
                          : Text(
                      ...
                      Text(
                        '${(m.value * 100).round()}%',
                        style: TextStyle(fontSize: s(11), color: kBodyTextColor),
                      ),
                    ],
                  ),
                  SizedBox(height: s(4)),
                  ...
                  if (isEditing)
                    Slider(
                      ...
                      onChanged: (val) {
                        final updated = List<MotiveItem>.from(motivations);
                        updated[idx] = MotiveItem(label: updated[idx].label, value: val);
                        motivationsNotifier.state = updated;
                      },
                      min: 0,
                      max: 1,
              ...
              child: Row(
                ...
                children: [
                  Padding(
                    ...
                    child: Icon(Icons.circle, size: s(6), color: kPrimaryColor),
                  ),
                  SizedBox(width: s(6)),
                  Expanded(
                    child: isEditing
                        ? Row(
                            children: [
                              Expanded(
                                child: TextField(
                                  controller: TextEditingController(text: f),
                                  onChanged: (val) {
                                    final updated = List<String>.from(frustrations);
                                    updated[idx] = val;
                                    frustrationsNotifier.state = updated;
                                  },
                                ),
                              ),
                              SizedBox(width: s(6)),
                              IconButton(
                                onPressed: () {
                                  final updated = List<String>.from(frustrations)..removeAt(idx);
                                  frustrationsNotifier.state = updated;
                                },
                                icon: Icon(Icons.delete, size: s(16), color: Colors.redAccent),
                              ),
                            ],
                          )
                        : Text(
                            f,
                            style: TextStyle(color: kBodyTextColor, fontSize: s(12)),
                          ),
                  ),
                ],
              ),
          ...
          if (isEditing)
            Row(
              children: [
                Expanded(
                  child: TextField(
                    controller: newFrustrationCtrl,
                    decoration: InputDecoration(
                      hintText: 'Add new frustration',
                      contentPadding: EdgeInsets.symmetric(horizontal: s(10), vertical: s(8)),
                    ),
                    onSubmitted: (val) {
                      final v = val.trim();
                      if (v.isEmpty) return;
                      final updated = List<String>.from(frustrations)..add(v);
                      frustrationsNotifier.state = updated;
                ...
                ElevatedButton(
                  onPressed: () {
                    final v = newFrustrationCtrl.text.trim();
                    if (v.isEmpty) return;
                    final updated = List<String>.from(frustrations)..add(v);
                    frustrationsNotifier.state = updated;
                    newFrustrationCtrl.clear();
                  ...
                  child: Text('Add', style: TextStyle(fontSize: s(12))),
                ),
              ],
            ),
        ],
  ...
  Widget _infoRow(
    ...
    return Row(
      children: [
        SizedBox(
          width: 80 * scale,
          child: Text('$label:', style: TextStyle(fontSize: 12 * scale)),
        ),
        Expanded(
          child: isEditing
              ? TextField(
                  controller: ctrl,
                  decoration: _inputDecoration(label, isDense: true, scale: scale)
                      .copyWith(contentPadding: EdgeInsets.symmetric(horizontal: 10 * scale, vertical: 10 * scale)),
                )
              : Text(
                  ctrl.text,
                  style: TextStyle(
        ...
        ElevatedButton(
          onPressed: () {
            ...
            if (newTag.isNotEmpty) {
              final updated = List<String>.from(ref.read(tagsprofilepageProvider));
              updated.add(newTag);
              ref.read(tagsprofilepageProvider.notifier).state = updated;
              controller.clear();
            }
  ...
  Widget _styledInterestChip(
    ...
    return Chip(
      label: Text(label, style: TextStyle(color: Colors.white, fontSize: s(10))),
      backgroundColor: color,
      onDeleted: onDeleted,
      deleteIconColor: Colors.white,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
      materialTapTargetSize: MaterialTapTargetSize.shrinkWrap,
      padding: EdgeInsets.symmetric(horizontal: s(6), vertical: s(8)),
    );
  }
    ...
    return Container(
      ...
      child: Tooltip(
        ...
        child: Container(
          ...
          height: 40 * scale,
          decoration: BoxDecoration(
            color: isActive ? kPrimaryColor.withOpacity(0.08) : Colors.transparent,
            borderRadius: BorderRadius.circular(8 * scale),
            border: isActive ? Border.all(color: kPrimaryColor, width: 1.2 * scale) : null,
          ),
          child: Icon(
            icon,
            color: isActive ? kPrimaryColor : kBodyTextColor,
            size: 18 * scale,
          ),
        ),
      ),
